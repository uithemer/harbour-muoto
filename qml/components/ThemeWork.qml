...
Item {
    ...
    function indexForPackName(packName) {
        ...
        for (var i = 0; i < themepackmodel.rowCount(); ++i) {
            ...
            if (pn === packName || pn === bare || pn === pref + bare || pn.replace(pref, "") === bare)
                return i;
    ...
    function beginManualThemeWork(progressBody) {
        ...
        app.showProgressNotification(qsTr("Applying theme"), progressBody, Notification.ProgressIndeterminate);
    }
    ...
    Connections {
        ...
        onIconProgress: {
            ...
            if (total <= 0) {
                app.showProgressNotification(qsTr("Applying theme"), qsTr("Waiting…"), Notification.ProgressIndeterminate);
                return ;
            }
            app.showProgressNotification(qsTr("Applying theme"), themeWork._progressBody, done / total);
        }
        onIconsApplied: {
    ...
    Timer {
        ...
        interval: 10000
        repeat: true
        running: themeWork.reloadActive && !settings.isRunning && Qt.application.state === Qt.ApplicationActive
