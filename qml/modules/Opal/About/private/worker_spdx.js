...
function sendError(spdxId, shortText) {
    WorkerScript.sendMessage({
        spdxId: spdxId, name: "", fullText: "", shortText: shortText, error: true
    });
}

function sendSuccess(spdxId, name, fullText, shortText) {
    WorkerScript.sendMessage({
        spdxId: spdxId, name: name, fullText: fullText, shortText: shortText, error: false
    });
}
...
function loadBundledThenRemote(message) {
    if (message.bundledUrl) {
        request("GET", message.bundledUrl, function(xhr) {
            try {
                var o = parseLicense(xhr);
                console.log(LOG_SCOPE, "license loaded from bundle", message.bundledUrl);
                sendSuccess(message.spdxId, o["name"], o["licenseText"],
                            getShortText(message.shortText, message.spdxId));
            } catch (e) {
                finishMissingCache(message);
            }
        }, function() {
            finishMissingCache(message);
        });
...
function finishMissingCache(message) {
    if (!!message.online) {
